...
# TRAITEMENT
# =====================================================
def run_rencontre_veterans(output=print, progress_callback=None):
    for i, (team_letter, div_id) in enumerate(TEAM_IDS.items(), 1):
        ...
        try:
            output(f"⬇️ Téléchargement HTML vétérans {team_letter}")
        ...
        output(f"➡️ {matchs_extraits} match(s) extraits pour vétérans {team_letter}\n")
        time.sleep(1.5)
    ...
    try:
        ...
        output(f"✅ {len(all_rows)} lignes envoyées à Google Sheets (vétérans).")
    except Exception as e:
...
def main(output=print, progress_callback=None):
    run_rencontre_veterans(output=output, progress_callback=progress_callback)
